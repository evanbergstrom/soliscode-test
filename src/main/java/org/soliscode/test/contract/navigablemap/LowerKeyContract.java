...
/// **Contract for the `lowerKey` method of a `NavigableMap`**
///
/// This interface defines tests for the [lowerKey(K)][NavigableMap#lowerKey] method.
/// It is designed to be used as a mix-in interface by test classes that verify [NavigableMap]
/// implementations.
///
/// ## Purpose
/// The purpose of this contract is to ensure that a navigable map's `lowerKey` implementation
/// correctly returns the greatest key strictly less than the given key, or `null` if there is
/// no such key.
///
/// ## Usage Examples
...
///
/// ```java
/// class MyNavigableMapLowerKeyTest implements LowerKeyContract<String, String, MyNavigableMap<String, String>> {
///     @Override
...
public interface LowerKeyContract<K, V, M extends NavigableMap<K, V>> extends MapContractSupport<K, V, M> {
    ...
    /// @since 1.0.0
    @Test
    @DisplayName("lowerKey(K) returns expected results when elements are present")
    default void lowerKey_whenNotEmpty_returnsExpectedResults() {
