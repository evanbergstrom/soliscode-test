...
/// **Contract for the `ceilingKey` method of a `NavigableMap`**
///
/// This interface defines tests for the [ceilingKey(K)][NavigableMap#ceilingKey] method.
/// It is designed to be used as a mix-in interface by test classes that verify [NavigableMap]
/// implementations.
///
/// ## Purpose
/// The purpose of this contract is to ensure that a navigable map's `ceilingKey` implementation
/// correctly returns the least key greater than or equal to the given key, or `null` if there is
/// no such key.
///
/// ## Usage Examples
...
///
/// ```java
/// class MyNavigableMapCeilingKeyTest implements CeilingKeyContract<String, String, MyNavigableMap<String, String>> {
///     @Override
...
/// @see NavigableMap#ceilingKey
/// @since 1.0.0
public interface CeilingKeyContract<K, V, M extends NavigableMap<K, V>> extends MapContractSupport<K, V, M> {

    /// Tests that the [ceilingKey][NavigableMap#ceilingKey] method can be called and
    ...
    /// @since 1.0.0
    @Test
    @DisplayName("ceilingKey(K) returns expected results when elements are present")
    default void ceilingKey_whenNotEmpty_returnsExpectedResults() {
