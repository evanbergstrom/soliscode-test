...
/// **Contract for the `floorEntry` method of a `NavigableMap`**
///
/// This interface defines tests for the [floorEntry(K)][NavigableMap#floorEntry] method.
/// It is designed to be used as a mix-in interface by test classes that verify [NavigableMap]
/// implementations.
///
/// ## Purpose
/// The purpose of this contract is to ensure that a navigable map's `floorEntry` implementation
/// correctly returns a key-value mapping associated with the greatest key less than or equal
...
///
/// ```java
/// class MyNavigableMapFloorEntryTest implements FloorEntryContract<String, String, MyNavigableMap<String, String>> {
///     @Override
...
/// @see NavigableMap#floorEntry
/// @since 1.0.0
public interface FloorEntryContract<K, V, M extends NavigableMap<K, V>> extends MapContractSupport<K, V, M> {

    /// Tests that the [floorEntry][NavigableMap#floorEntry] method can be called and
    ...
    /// @since 1.0.0
    @Test
    @DisplayName("floorEntry(K) returns expected results when elements are present")
    default void floorEntry_whenNotEmpty_returnsExpectedResults() {
