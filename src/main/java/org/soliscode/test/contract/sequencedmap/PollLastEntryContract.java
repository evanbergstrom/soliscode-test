...
/// Contract for the `pollLastEntry()` method of [SequencedMap].
///
/// ### Purpose
/// Verifies that the `pollLastEntry()` method correctly removes and returns the last entry
...
/// #### Implementation
/// ```java
/// public class MySequencedMapTest implements PollLastEntryContract<String, String, MySequencedMap<String, String>> {
///     @Override
...
/// @see SequencedMap#pollLastEntry()
/// @author evanbergstrom
/// @since 1.0.0
public interface PollLastEntryContract<K, V, M extends SequencedMap<K, V>> extends MapContractSupport<K, V, M> {

    /// Verifies that `pollLastEntry()` can be called successfully or throws [UnsupportedOperationException]
    ...
    /// @since 1.0.0
    @Test
    @DisplayName("pollLastEntry: when called, is successful or throws UnsupportedOperationException")
    default void pollLastEntry_whenCalled_isSuccessful() {
