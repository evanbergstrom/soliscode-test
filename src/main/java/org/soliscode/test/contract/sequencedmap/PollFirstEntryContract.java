...
/// Contract for the `pollFirstEntry()` method of [SequencedMap].
///
/// ### Purpose
/// Verifies that the `pollFirstEntry()` method correctly removes and returns the first entry
/// in the map's sequencing order, or handles the empty case and unsupported operations appropriately.
///
/// ### Usage Examples
///
/// #### Implementation
/// ```java
/// public class MySequencedMapTest implements PollFirstEntryContract<String, String, MySequencedMap<String, String>> {
///     @Override
...
/// @see SequencedMap#pollFirstEntry()
/// @author evanbergstrom
/// @since 1.0.0
public interface PollFirstEntryContract<K, V, M extends SequencedMap<K, V>> extends MapContractSupport<K, V, M> {

    /// Verifies that `pollFirstEntry()` can be called successfully or throws [UnsupportedOperationException]
    ...
    /// @since 1.0.0
    @Test
    @DisplayName("pollFirstEntry: when called, is successful or throws UnsupportedOperationException")
    default void pollFirstEntry_whenCalled_isSuccessful() {
