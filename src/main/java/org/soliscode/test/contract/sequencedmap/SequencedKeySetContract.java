...
/// Contract for the `sequencedKeySet()` method of [SequencedMap].
///
/// ### Purpose
/// Verifies that the `sequencedKeySet()` method correctly returns a sequenced set view
/// of the map's keys, or handles unsupported operations appropriately.
///
/// ### Usage Examples
///
/// #### Implementation
/// ```java
/// public class MySequencedMapTest implements SequencedKeySetContract<String, String, MySequencedMap<String, String>> {
///     @Override
...
public interface SequencedKeySetContract<K, V, M extends SequencedMap<K, V>> extends MapContractSupport<K, V, M> {
    ...
    /// @since 1.0.0
    @Test
    @DisplayName("sequencedKeySet: when called, returns sequenced set or throws UnsupportedOperationException")
    default void sequencedKeySet_whenCalled_returnsSequencedSet() {
